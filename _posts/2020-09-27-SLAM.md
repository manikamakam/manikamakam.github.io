...
Later, it was proved that the D-opt uncertainty criteria can be used for solving active SLAM by writing the formula for it as shown in the table. It's proved in (link to paper) that D-opt performs comparably to A-opt metric as popularized in (link to papers). 
